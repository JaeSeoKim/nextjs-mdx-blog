...
> = ({ children, style, className }) => {
  ...
  // Show/hide animation
  React.useEffect(() => {
    if (visualState === VisualState.showing || shouldReducedMotion) {
      return;
    }
  ...
  }, [options, visualState, enterMs, exitMs, shouldReducedMotion]);
      ...
      const ro = new ResizeObserver(entries => {
        for (let entry of entries) {
          ...
          outer.animate(
            ...
            {
              duration: shouldReducedMotion ? 0 : enterMs / 2,
              easing: "ease-out",
              fill: "forwards",
            },
          );
  ...
  }, [visualState, options, enterMs, exitMs, shouldReducedMotion]);

  // Bump animation between nested actions
  const firstRender = React.useRef(true);
  React.useEffect(() => {
    if (firstRender.current || shouldReducedMotion) {
  ...
  }, [currentRootActionId, enterMs, shouldReducedMotion]);
  ...
  return (
    <div
      ...
      style={{
        ...(shouldReducedMotion
          ? appearanceAnimationKeyframes[2]
